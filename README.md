...
- [x] Rename artifacts and plugin IDs (see [coordinates](#coordinates-old--new)); **Java packages
...
The organisation (`github.com/openjavaformat`) is registered, and the Maven namespace
`dev.openjavaformat` is submitted for verification on the Central Portal. The build already uses
every name below — for its Gradle projects and directories as well as for what it publishes.
...
1. ~~**Maven group and GitHub org.**~~ **Resolved:** the `openjavaformat` GitHub organisation is
   registered, and the group id is `dev.openjavaformat`, submitted for verification on the Maven
   Central Portal. Publishing waits for that verification.
...
1. Clone this repo
2. run `./gradlew :open-java-format-idea-plugin:build`
3. In IntelliJ, install a plugin from disk. Build artifacts are located in `./open-java-format-idea-plugin/build/distributions/`

![Install plugin from disk](./docs/images/install_plugin_from_disk.png)

## Eclipse plugin

See [open-java-format-eclipse-plugin](./open-java-format-eclipse-plugin).
